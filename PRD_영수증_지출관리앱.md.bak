...
| # | 작업 | 산출물 | 우선도 | 상태 |
|---|------|--------|--------|------|
| 1-1 | GitHub 레포지토리 생성 및 `.gitignore` 설정 | `receipt-tracker` 레포 | Must | ⬜ 수동 필요 |
| 1-2 | 프로젝트 디렉토리 구조 생성 | `backend/`, `frontend/` 폴더 | Must | ✅ 완료 |
| 1-3 | `.env` 파일 생성 및 `UPSTAGE_API_KEY` 등록 | `.env`, `.env.example` | Must | ✅ 완료 |
| 1-4 | Python 가상환경 생성 및 패키지 설치 | `venv/`, `requirements.txt` | Must | ✅ 완료 |
| 1-5 | FastAPI 앱 뼈대 생성 | `backend/main.py` (CORS 포함) | Must | ✅ 완료 |
| 1-6 | expenses.json 초기 파일 생성 | `backend/data/expenses.json` → `[]` | Must | ✅ 완료 |

> **참고**: 1-1 GitHub 레포 생성은 수동 작업 필요. 나머지는 모두 완료.
...
├── routers/                 # Phase 2에서 구현
├── services/                # Phase 2에서 구현
├── data/
│   └── expenses.json        # 빈 배열 [] 초기화
└── requirements.txt
frontend/                    # Phase 4에서 구현
.env                         # UPSTAGE_API_KEY 설정됨
.env.example                 # API 키 템플릿
.gitignore                   # .env, venv/, __pycache__ 등 포함
venv/                        # Python 3.13 가상환경
```

#### requirements.txt

> **변경 사항**: Python 3.13 환경에서 `langchain==0.2.0`이 요구하는 `numpy<2`의 prebuilt wheel이 없어 빌드 실패. 최신 호환 버전으로 업데이트.
...
*v1.1: 화면 디자인 & 스타일 가이드(섹션 10) 추가 | v1.2: 개발 일정 Phase별 세분화(섹션 13) 추가 | v1.3: 바이브 코딩 3원칙 적용(섹션 13) 추가 | v1.4: Phase 1 완료 반영 — requirements.txt Python 3.13 호환 버전 업데이트(langchain 1.2.15 외), 완료 체크리스트 및 실제 생성 파일 목록 갱신*
